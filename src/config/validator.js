...
function validateNames() {
  ...
  for (let parameterName of parametersName) {
    if (!process.env[parameterName]) {
      console.error(
        `Ошибка файла с переменными окружения (.env)! Параметр: ${parameterName} не найден!`
      );
      isSuccess = false;
    }
  }
...
function validateNumbers() {
  ...
  for (let parameterName of numberParametersName) {
    if (isNaN(parseInt(process.env[parameterName]))) {
      console.error(
        `Ошибка файла с переменными окружения (.env)! Параметр: ${parameterName} не является числом!`
      );
      isSuccess = false;
    }
  }
