...
export function parseParameters() {
  ...
  if (process.env.npm_lifecycle_event == 'withEnv') {
    const parsingDaysResult = parseDays(
      config.days['parameterName'],
      process.env.DAYS
    );
    ...
    const parsingCitiesResult = parseCities(
      config.city['parameterName'],
      process.env.CITY
    );
  ...
  if (parameters.cities.length == 0) {
    ...
    console.error(
      `Ошибка! Для параметра ${parameterName} не указано значение!`
    );
    process.exit(1);
  }
  ...
  if (currentParameter === parameterName) {
    if (nextParameter) {
      ...
      if (
        ...
        convertedValue < config.days.minValue ||
        convertedValue > config.days.maxValue ||
        !Number.isInteger(Number(nextParameter))
      ) {
        console.error(
          `Ошибка! Параметр ${parameterName} должен быть целым числом в диапазоне от ${config.days.minValue} до ${config.days.maxValue} включительно.`
        );
        process.exit(1);
      } else {
        result.success = true;
